...
glm_stunting <- svyglm(as_formula(paste0("c_stunting ~ factor(e_interaction)*c_male",region_covariates)),design = analytic_survey,
                       family = quasipoisson())

glm_underweight <- svyglm(as_formula(paste0("c_underweight ~ factor(e_interaction)*c_male",region_covariates)),design = analytic_survey,
                          family = quasipoisson())

glm_wasting <- svyglm(as_formula(paste0("c_wasting ~ factor(e_interaction)*c_male",region_covariates)),design = analytic_survey,
                      family = quasipoisson())



saveRDS(save_svyglm(glm_stunting) ,paste0(path_lockdown_folder,"/working/nlxb/nlxb02_glm_stunting.RDS"))
saveRDS(save_svyglm(glm_underweight),paste0(path_lockdown_folder,"/working/nlxb/nlxb02_glm_underweight.RDS"))
saveRDS(save_svyglm(glm_wasting),paste0(path_lockdown_folder,"/working/nlxb/nlxb02_glm_wasting.RDS"))
...
summary_poisson %>% 
  ...
  write_csv(.,"models sequential sex/nlxb02_poisson outcomes x economic shocks in urban.csv")

e_interaction_level <- c(2:19)

marginal_predictions_male <- map_dfr(e_interaction_level,function(e){
  print(e);
  e0_df = analytic_sample %>% 
    dplyr::filter(c_male == 1) %>% 
...
marginal_predictions_female <- map_dfr(e_interaction_level,function(e){
  print(e);
  e0_df = analytic_sample %>% 
    dplyr::filter(c_male == 0) %>% 
    mutate_at(vars(e_interaction),function(x) case_when(TRUE ~ 1));
  e1_df = analytic_sample %>% 
    dplyr::filter(c_male == 0) %>% 
    mutate_at(vars(e_interaction),function(x) case_when(TRUE ~ e));
...
bind_rows(marginal_predictions_male %>% mutate(c_male = 1),
          marginal_predictions_female %>% mutate(c_male = 0)) %>% 
write_csv(.,paste0("models sequential sex/nlxb02_marginal predictions from poisson for urban.csv"))
...
#             by=c(names(overlaps_unique)[-11]))
# 
# pred_overlaps_male <- map_dfr(1:nrow(overlaps), function(i){
#   print(i);
...
# 
# 
# pred_overlaps_female <- map_dfr(1:nrow(overlaps), function(i){
#   print(i);
#   e_df = analytic_sample %>% 
#     dplyr::filter(c_male == 0) %>% 
#   dplyr::select(-ends_with("_d"),-e_interaction) %>% 
...
# })
# 
# bind_rows(bind_cols(overlaps,pred_overlaps_male) %>% mutate(c_male = 1),
#           bind_cols(overlaps,pred_overlaps_female) %>% mutate(c_male = 0)) %>% 
# 
#   write_csv(.,"models sequential sex/nlxb02_prediction for unique months and year for urban.csv")
