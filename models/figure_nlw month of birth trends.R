...
overlap_predictions <- bind_rows(
  read_csv("models/nlwb02_prediction for unique months and year for urban.csv") %>% mutate(m_rural = "Urban"),
  read_csv("models/nlwb03_prediction for unique months and year for rural.csv") %>% mutate(m_rural = "Rural")) %>% 
