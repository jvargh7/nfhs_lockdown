...
glm_stunting <- svyglm(as_formula(paste0("c_stunting ~ sum_e1_d + sum_e2_d",total_covariates)),design = analytic_survey,
                       family = quasipoisson())

glm_underweight <- svyglm(as_formula(paste0("c_underweight ~ sum_e1_d + sum_e2_d",total_covariates)),design = analytic_survey,
                          family = quasipoisson())

glm_wasting <- svyglm(as_formula(paste0("c_wasting ~ sum_e1_d + sum_e2_d",total_covariates)),design = analytic_survey,
                      family = quasipoisson())
...
summary_poisson %>% 
  ...
  write_csv(.,"models/nlwa01_poisson outcomes x economic shocks in total.csv")
...
write_csv(marginal_predictions,paste0("models/nlwa01_marginal predictions from poisson for total.csv"))
...
#   write_csv(.,"models/nlwa01_prediction for unique months and year for total.csv")
# 
# 
