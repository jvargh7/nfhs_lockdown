...
analytic_sample <- readRDS(paste0(path_lockdown_folder,"/working/analytic_sample.RDS")) %>% 
  ...
  glm_stunting <- svyglm(c_stunting ~ ns(c_age,df=4) + e1_p1_d + e1_p2_d + e1_p3_d + e1_p4_d + e1_p5_d +
                           ...
                           m_caste + m_wealthq + m_religion + m_age + m_education + m_alcohol + m_smoking + factor(v024_nfhs5) + factor(c_month),design = analytic_survey,
                         family = poisson())
...
summary_poisson %>% 
  ...
  write_csv(.,"paper/nlmb02_poisson outcomes x economic shocks in urban.csv")
...
write_csv(marginal_predictions,paste0("paper/nlmb02_marginal predictions from poisson for urban.csv"))
...
bind_cols(overlaps,pred_overlaps) %>% 
  write_csv(.,"paper/nlmb02_prediction for unique months and year for urban.csv")
