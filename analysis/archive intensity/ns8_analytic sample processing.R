...
predelta_svy <- analytic_sample %>% 
  dplyr::filter(c_interview < delta_start) %>% 
  as_survey_design(ids = v021,strata=v023,weights=sampleweight,variance ="YG",pps="brewer")

predelta2_svy <- analytic_sample %>%
  dplyr::filter(age_le24 ==1 | age_ge36 == 1,c_interview < delta_start) %>% 
  as_survey_design(ids = v021,strata=v023,weights=sampleweight,variance ="YG",pps="brewer")
