...
analytic_sample <- nlma_analytic_sample %>% 
  dplyr::filter(m_rural == 1,nfhs5==1)
...
saveRDS(save_svyglm(glm_stunting) ,paste0(path_lockdown_folder,"/working/nls5/nls501r_glm_stunting.RDS"))
saveRDS(save_svyglm(glm_underweight),paste0(path_lockdown_folder,"/working/nls5/nls501r_glm_underweight.RDS"))
saveRDS(save_svyglm(glm_wasting),paste0(path_lockdown_folder,"/working/nls5/nls501r_glm_wasting.RDS"))
# Stunting -----------
...
summary_poisson %>% 
  ...
  write_csv(.,"sensitivity nfhs5/nls501r_summary poisson regression rural.csv")
