...
analytic_sample <- nlma_analytic_sample %>% 
  dplyr::filter(m_rural == 1,c_male == 1)
...
saveRDS(save_svyglm(glm_stunting) ,paste0(path_lockdown_folder,"/working/nlxb/nlxb01r_glm_stunting.RDS"))
saveRDS(save_svyglm(glm_underweight),paste0(path_lockdown_folder,"/working/nlxb/nlxb01r_glm_underweight.RDS"))
saveRDS(save_svyglm(glm_wasting),paste0(path_lockdown_folder,"/working/nlxb/nlxb01r_glm_wasting.RDS"))
# Stunting -----------
...
summary_poisson %>% 
  ...
  write_csv(.,"sensitivity boys/nlxb01r_summary poisson boys rural.csv")
