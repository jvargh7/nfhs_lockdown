...
nfhs5_exposure %>% 
  dplyr::filter(!v024 %in% v024_nfhs5_14states) %>% 
...
analytic_sample_s3p2 <- analytic_sample_s3 %>% 
  ...
  bind_cols(
    future_pmap_dfr(.,.f=function(c_dob,v024,sdist,c_interview,...){
      tryCatch({
        average_mobility_restriction(c_dob,v024,sdist,c_interview)},
        error = function(e){
