...
demonetization_start_sens <- "2016-11-09"
demonetization_stop_sens <- "2017-11-08"

overlaps <- data.frame(dates = seq.Date(as_date("2010-02-02"),as_date("2021-04-30"),by=1),
                       d_start = as_date(demonetization_start_sens),
                       d_stop = as_date(demonetization_stop_sens),
...
list_overlaps <- map_dfr(1:nrow(overlaps),
                         function(i){
                           ...
                           e1_p1 = Overlap(c(dob-38*7,dob),
                                           c(as_date(demonetization_start_sens),as_date(demonetization_stop_sens)));
...
bind_cols(overlaps,list_overlaps) %>% 
  mutate_at(vars(starts_with("e")),.f = list(d = function(x) case_when(x > 90 ~ 1,
                                                                       TRUE ~ 0))) %>%
  saveRDS(.,paste0("data/overlaps sensitivity.RDS"))