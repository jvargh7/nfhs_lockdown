...
analytic_sample <- nlma_analytic_sample %>% 
  dplyr::filter(m_rural == 0,c_male == 0)
...
saveRDS(save_svyglm(glm_stunting) ,paste0(path_lockdown_folder,"/working/nlxg/nlxg01u_glm_stunting.RDS"))
saveRDS(save_svyglm(glm_underweight),paste0(path_lockdown_folder,"/working/nlxg/nlxg01u_glm_underweight.RDS"))
saveRDS(save_svyglm(glm_wasting),paste0(path_lockdown_folder,"/working/nlxg/nlxg01u_glm_wasting.RDS"))
# Stunting -----------
...
summary_poisson %>% 
  ...
  write_csv(.,"sensitivity boys/nlxg01u_summary poisson girls urban.csv")
