...
overlap_predictions <- bind_rows(
  read_csv("models sequential sex/nlxb02_prediction for unique months and year for urban.csv") %>% mutate(m_rural = "Urban"),
  read_csv("models sequential sex/nlxb03_prediction for unique months and year for rural.csv") %>% mutate(m_rural = "Rural")) %>% 
...
ggarrange(figA,
  ...
  ggsave(.,filename=paste0(path_lockdown_folder,"/figures/nlx_month of birth trends.png"),width=8,height = 10)
