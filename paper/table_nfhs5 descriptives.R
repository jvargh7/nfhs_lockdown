nfhs5_sample <- readRDS(paste0(path_lockdown_folder,"/working/analytic_sample.RDS")) %>% 
  dplyr::filter(v024_nfhs5 %in% v024_nfhs5_14states,nfhs5==1)
require(srvyr)

nfhs5_svydesign <- nfhs5_sample %>% 
...
nfhs5_continuous <- nfhs5_svydesign %>% 
  group_by(m_rural,phase) %>% 
  summarize_at(vars(one_of(continuous_variables)),
               .funs = list(mean = ~survey_mean(.,na.rm=TRUE,vartype="ci"))) %>% 
  pivot_longer(cols=-one_of("phase","m_rural"),names_to=c("variable","est"),names_sep ="_mean") %>% 
...
nfhs5_groups <- map_dfr(group_variables,
                        function(g){
                          ...
                          nfhs5_svydesign %>% 
                            group_by_at(vars(m_rural,phase,g)) %>% 
...
nfhs5_counts <- nfhs5_sample %>% 
  ...
  pivot_longer(names_to="variable",values_to="n",cols=-one_of("phase","m_rural")) %>% 
