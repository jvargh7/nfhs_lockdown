...
nlsz_outcomes <- left_join(read_csv("sensitivity zscores/nlsz01u_summary gaussian regression urban.csv") %>% 
                             rename_at(vars(HAZ,WAZ,WHZ),~paste0("Urban_",.)),
                           read_csv("sensitivity zscores/nlsz01r_summary gaussian regression rural.csv") %>% 
                             rename_at(vars(HAZ,WAZ,WHZ),~paste0("Rural_",.)),
                           by = "term") %>% 
...
write_csv(nlsz_outcomes,("paper/nlsz_coefficients.csv"))
