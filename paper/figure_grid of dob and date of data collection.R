...
figA <- grid_df %>% 
  ...
  # geom_hline(yintercept = as_date(demonetization_start),col="red",linetype=2) +
  # geom_hline(yintercept = as_date(demonetization_stop),col="red",linetype=2) +
  # geom_hline(yintercept = as_date(lockdown_start),col="blue",linetype=2) +
  # geom_hline(yintercept = as_date(lockdown_stop),col="blue",linetype=2) +
  # geom_rect(xmin = as_date(demonetization_start),xmax = as_date(demonetization_stop),ymin = as_date("2010-01-01"),ymax = as_date("2022-01-01"), fill="grey80",alpha=0.01,col=NA) +
  # geom_rect(xmin = as_date(lockdown_start),xmax = as_date(lockdown_stop),ymin = as_date("2010-01-01"),ymax = as_date("2022-01-01"), fill="grey80",alpha=0.01,col=NA)
...
ggarrange(figA,figB,
  ...
  ggsave(.,filename=paste0(path_lockdown_folder,"/figures/figure_grid of dob and data collection for prevalence.png"),width = 10,height=10)
