...
write_csv(summary_e_interaction_region,("paper/summary nlsy urban rural counts by exposure.csv"))
...
nlsy_outcomes <- left_join(read_csv("sensitivity young/nlsy01u_summary poisson regression urban.csv") %>% 
                             rename_at(vars(Stunting,Underweight,Wasting),~paste0("Urban_",.)),
                           read_csv("sensitivity young/nlsy01r_summary poisson regression rural.csv") %>% 
...
write_csv(nlsy_outcomes,("paper/nlsy_coefficients.csv"))
