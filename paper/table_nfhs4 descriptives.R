...
nfhs4_continuous <- nfhs4_svydesign %>% 
  ...
  pivot_longer(.,names_to=c("variable","est"),names_sep ="_mean",cols=-m_rural) %>% 
...
nfhs4_groups <- map_dfr(group_variables,
                                 function(g){
                                   ...
                                   nfhs4_svydesign %>% 
                                     group_by_at(vars(one_of("m_rural",g))) %>% 
...
nfhs4_counts <- nfhs4_sample %>% 
  ...
  pivot_longer(names_to="variable",values_to="n",cols=-m_rural) %>% 
