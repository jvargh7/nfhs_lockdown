...
nlma_outcomes <- left_join(read_csv("sensitivity age spline/nlsb01u_summary poisson regression urban.csv") %>% 
                               rename_at(vars(Stunting,Underweight,Wasting),~paste0("Urban_",.)),
                             read_csv("sensitivity age spline/nlsb01r_summary poisson regression rural.csv") %>% 
...
write_csv(nlma_outcomes,("paper/nlsb_coefficients.csv"))
