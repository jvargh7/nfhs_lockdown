...
summary_e_interaction_region <- readRDS(paste0(path_lockdown_folder,"/working/analytic_sample.RDS")) %>% 
  ...
  left_join(overlaps_unique,
            by=c(names(overlaps_unique)[-11])) %>% 
  mutate(year_categories = case_when(year(c_dob) %in% c(2017,2018) ~ "2017-18",
                                     TRUE ~ "2010-15")) %>% 
...
write_csv(summary_e_interaction_region,("paper/summary nlsc urban rural counts by exposure.csv"))
...
nlsc_outcomes <- left_join(read_csv("sensitivity conception/nlsc01u_summary poisson regression urban.csv") %>% 
                             rename_at(vars(Stunting,Underweight,Wasting),~paste0("Urban_",.)),
                           read_csv("sensitivity conception/nlsc01r_summary poisson regression rural.csv") %>% 
...
write_csv(nlsc_outcomes,("paper/nlsc_coefficients.csv"))
