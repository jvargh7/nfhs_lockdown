...
summary_e_interaction <- readRDS(paste0(path_lockdown_folder,"/working/nlsd/nlsd_analytic_sample.RDS")) %>% 
  group_by(e_interaction) %>% 
  dplyr::summarise(daterange = paste0(format(min(c_dob),"%d-%m-%Y")," to ",format(max(c_dob),"%d-%m-%Y")))

summary_e_interaction_region <- readRDS(paste0(path_lockdown_folder,"/working/nlsd/nlsd_analytic_sample.RDS")) %>% 
...
write_csv(summary_e_interaction_region,("paper/nlsd urban rural counts by exposure.csv"))


readRDS(paste0(path_lockdown_folder,"/working/nlsd/nlsd_analytic_sample.RDS")) %>%
...
nlsd_outcomes <- left_join(read_csv("sensitivity demonetization/nlsd01u_summary poisson regression urban.csv") %>% 
                             rename_at(vars(Stunting,Underweight,Wasting),~paste0("Urban_",.)),
                           read_csv("sensitivity demonetization/nlsd01r_summary poisson regression rural.csv") %>% 
                             rename_at(vars(Stunting,Underweight
                                            # Wasting
                                            ),~paste0("Rural_",.)),
                           by = "term") %>% 
  ...
  mutate(term2 = factor(term,labels=paste0("Table Order ",sprintf("%02d",c(10:2)))) %>% as.character(.)) %>% 
  arrange(term2)




write_csv(nlsd_outcomes,("paper/nlsd_coefficients.csv"))
