...
overlap_predictions <- bind_rows(
  read_csv("models with sequential interactions/nlsb02_prediction for unique months and year for urban.csv") %>% mutate(m_rural = "Urban"),
  read_csv("models with sequential interactions/nlsb03_prediction for unique months and year for rural.csv") %>% mutate(m_rural = "Rural")) %>% 
...
ggarrange(figA,
  ...
  ggsave(.,filename=paste0(path_lockdown_folder,"/figures/nls_month of birth trends.png"),width=8,height = 10)
