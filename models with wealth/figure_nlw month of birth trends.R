...
overlap_predictions <- bind_rows(
  read_csv("models with wealth/nlwb02_prediction for unique months and year for urban.csv") %>% mutate(m_rural = "Urban"),
  read_csv("models with wealth/nlwb03_prediction for unique months and year for rural.csv") %>% mutate(m_rural = "Rural")) %>% 
...
ggarrange(figA,
  ...
  ggsave(.,filename=paste0(path_lockdown_folder,"/figures/nlwb_month of birth trends.png"),width=8,height = 10)
