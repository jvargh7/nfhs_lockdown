...
glm_stunting <- svyglm(c_stunting ~ ns(c_age,df=4) + sum_e1_d + sum_e2_d + nfhs5 + c_male + 
                         m_wealthq + m_caste + m_religion + m_age + m_education + factor(sdistri) + factor(c_month),design = analytic_survey,
                       family = poisson())

glm_underweight <- svyglm(c_underweight ~ ns(c_age,df=4) + sum_e1_d + sum_e2_d + nfhs5 + c_male + 
                            m_wealthq + m_caste + m_religion + m_age + m_education + factor(sdistri) + factor(c_month),design = analytic_survey,
                          family = poisson())

glm_wasting <- svyglm(c_wasting ~ ns(c_age,df=4) + sum_e1_d + sum_e2_d + nfhs5 + c_male + 
                        m_wealthq + m_caste + m_religion + m_age + m_education + factor(sdistri) + factor(c_month),design = analytic_survey,
                      family = poisson())
...
summary_poisson %>% 
  ...
  write_csv(.,"models with wealth/nlwa02_poisson outcomes x economic shocks in urban.csv")
...
write_csv(marginal_predictions,paste0("models with wealth/nlwa02_marginal predictions from poisson for urban.csv"))
...
#   write_csv(.,"models with wealth/nlwa02_prediction for unique months and year for urban.csv")
# 
# 
