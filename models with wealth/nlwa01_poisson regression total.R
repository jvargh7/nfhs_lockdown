...
glm_stunting <- svyglm(c_stunting ~ ns(c_age,df=4) + sum_e1_d + sum_e2_d + nfhs5 + c_male + 
                         m_caste + m_religion + m_age + m_education + m_rural + factor(sdistri) + factor(c_month),design = analytic_survey,
                       family = poisson())
...
summary_poisson %>% 
  ...
  write_csv(.,"paper/nlma01_poisson outcomes x economic shocks in total.csv")
...
write_csv(marginal_predictions,paste0("paper/nlma01_marginal predictions from poisson for total.csv"))
...
#   write_csv(.,"paper/nlma01_prediction for unique months and year for total.csv")
# 
# 
